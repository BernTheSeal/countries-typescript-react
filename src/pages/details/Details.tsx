      ...
      const fetchData = async () => {
        try {
          ...
          const borders = data.filter((country) =>
            currentCountry[0].borders.includes(country.cca3)
          );
          setBorders(borders);
        } catch (error) {
          console.log(error);
    ...
    const getWeatherData = async () => {
      try {
        const data = await axios.get<any>(
          ...
          }&q=${currentCountry[0].capital}&days=1&aqi=yes&alerts=yes`
        );
        setWeatherData(data);
      } catch (error) {
        console.log(error);
        ...
        <div className="details-page">
          ...
          <div className="details-page-population">
            ...
            <div className="details-page-population-rank">
              ...
              <p className="details-page-population-rank-description">{rank}</p>
            </div>
          </div>
              ...
              <ol>
                {daysOfWeek.map((day, index) => (
                  <div className="details-page-startofweek-description-calendar">
                    <p className="day">{day.abbreviation}</p>
                    <div className="day_decimal">
                      <p
                        key={index}
                        className={
                          day.name.toLowerCase() ===
                          currentCountry[0].startOfWeek
                            ? "day-active"
                            : ""
                        }
                      >
            ...
            <div className="details-page-check-independent">
              ...
              <div className="details-page-check-independent-description">
                {currentCountry[0].independent ? (
                  <i
                    style={{ color: "#248939", fontSize: "18px" }}
                  ...
                  <i
                    style={{ color: "#b03535", fontSize: "18px" }}
              ...
              <ul className="details-page-borders-description">
                {borders.length > 0
                  ? borders.map((border: any, index: any) => (
                      <li
                        key={index}
                        className="details-page-borders-description-country"
                      >
                        <div>
                        ...
                        <div className="hidden">
                          <i className="fa-solid fa-mountain-sun"></i>
                          <p>
                            {border.area.toLocaleString()} km<sup>2</sup>
                          </p>
                        </div>
                        <div className="hidden">
                          <i className="fa-solid fa-globe"></i>
                          <p>{border.region}</p> ,<p>{border.subregion}</p>
                        </div>
                      </li>
                    ))
